...
def naive_bayes(training1_sentence, training2_sentence, testing_sentence, alpha, prob1, prob2):
    # Exercise
    bow1 = create_BOW(training1_sentence)
    bow2 = create_BOW(training2_sentence)
    test = create_BOW(testing_sentence)

    classify1 = calculate_doc_prob(bow1, test, alpha) + math.log(prob1)
    classify2 = calculate_doc_prob(bow2, test, alpha) + math.log(prob2)
